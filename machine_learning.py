...
class machine_learning:
    ...
    def multi_regr_with_scikit(self, num_iteration=50):
        ...
        for i in range(int(num_iteration)):
            ...
            x_train, x_test, y_train, y_test = train_test_split(
                self.features, self.target, random_state=i
            )
            xt_train, xt_test, yt_train, yt_test = train_test_split(
                self.t_features, self.t_target, random_state=i
            )
        ...
        data = pd.DataFrame(
            {
                "coef": np.around(model.coef_, 4),
                "average": [self.features[i].mean() for i in self.features.columns],
            },
            index=self.features.columns,
        )

        # evaluating features effect on price
        data["Effect on price"] = [
            data.iloc[i, 0] * data.iloc[i, 1] for i in range(len(data.index))
        ]
    ...
    def multi_regr_with_lstat(self, num_iteration=50):
        ...
        Effect on price : Effect on price = coef * average.
          This index is a measurment to evaluate the effect of each feature on price.
        the bigger absolute value of this number, the more effect this feature has on price.
        ...
        Transformed model accuracy
        BIC of the model
        AIC of the model
        short_summary : a pandas dataframe contains P-values, variance inflation factor,
          coeficients,etc. of the different column.
        long_summary : a detailed summary of our model based on inputs which is created by statsmodel library.
        ...
        for i in range(int(num_iteration)):
            ...
            short_summary = pd.DataFrame(
                {"coef": results.params, "p-values": round(results.pvalues, 3)}
            )

            # VIF Factor column to store variance inflation factor for each column
            vif = [
                round(variance_inflation_factor(x_train.values, i), 3)
        ...
        print(
            "r-squared for modified dataframe is: ", transformed_model / num_iteration
        )
    ...
    def multi_regr_with_randomforest(self, num_iteration=50):
        """
        This function uses Scikit-Learn Ensemble random forrest regression model to perform
        multivariate regression on our dataset.
        It takes two dataframe as the input.
        It prints the accuracy for both dataframes.
...
def feature_target_splitter(df):
    """
    This function takes the dataframe and returns features and targets
    """
    # Creating Features and Targets
    target = df["SalePrice"]
    features = df.drop("SalePrice", axis=1)
...
def multireg_statmodels(df):
    t_features, t_target = feature_target_splitter(df)
    xt_train, xt_test, yt_train, yt_test = train_test_split(
        t_features, t_target, random_state=2200
    )

    # Same steps as above for transformed dataset
    x_incl_const = sm.add_constant(xt_train)
    t_features_const = sm.add_constant(t_features)
    model = sm.OLS(yt_train, x_incl_const)
    results = model.fit()
    predictions = results.predict(t_features_const)

    return t_target, predictions
