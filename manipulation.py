...
def box_cox_transformer(df):
    ...
    skew_dataframe = pd.DataFrame(
        index=skewed_features,
        data={
            "Original Data Skew": initial_skew_list,
            "Transformed Data Skew": transformed_skew_list,
        },
    )
...
def calculate_z_scores(series):
    """
    This function take population in form of a pandas series, then calculate the z score of each observation
    """
...
def outlayer_remover(df, zscores):
    """
    ...
    print(f"{len(outlayers)} Houses are detected as outlayers")
